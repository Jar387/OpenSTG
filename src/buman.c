...
static void style_trace_task(int looptime, void* data){
...
static void style_default_task(int looptime, void *data)
{
	...
	if(buman->way==1){
		...
		start_angle = buman->dir-(buman->dif)*((buman->way)/2-1)-((buman->dif)/2);
	}
	for(int i=0;i<buman->way;i++){
		bullet *bu = gen_bullet(buman->color, buman->type, buman->offset);
...
void fire(int id){
	...
	buman_list[id].spd_delta = (buman_list[id].last_v-buman_list[id].first_v)/buman_list[id].count;
	switch(buman_list[id].style){
		case DEFAULT: add_periodic_times_task(8, buman_list[id].count, &style_default_task, &buman_list[id]); break;
		case LAYER_DIF: add_periodic_times_task(8, buman_list[id].count, &style_layer_dif_task, &buman_list[id]); break;;
		default: ILLEGALPARAM("bullet manager style"); return;
	}
...
void boffset(int id, int offset_x, int offset_y){
	buman_list[id].offset = (v2d){offset_x, offset_y};
}
